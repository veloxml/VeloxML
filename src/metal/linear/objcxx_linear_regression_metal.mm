...
- (instancetype)init {
    ...
    if (self) {
        ...
        dispatch_data_t metalData = dispatch_data_create(kernelData.bytes, kernelData.length, DISPATCH_DATA_DESTRUCTOR_DEFAULT, NULL);
        ...
        id<MTLFunction> function = [_library newFunctionWithName:@"lu_decomposition_optim"];
...
- (void)fitWithX:(const std::vector<float>&)X y:(const std::vector<float>&)y rows:(int)rows cols:(int)cols {
    int N = cols + 1;
    ...
    id<MTLBuffer> bufferA = [_device newBufferWithBytes:A.data() length:N*N*sizeof(float) options:MTLResourceStorageModeShared];
    id<MTLBuffer> bufferL = [_device newBufferWithLength:N*N*sizeof(float) options:MTLResourceStorageModeShared];
    id<MTLBuffer> bufferU = [_device newBufferWithLength:N*N*sizeof(float) options:MTLResourceStorageModeShared];
...
- (std::vector<float>)predictWithX:(const std::vector<float>&)X rows:(int)rows cols:(int)cols {
